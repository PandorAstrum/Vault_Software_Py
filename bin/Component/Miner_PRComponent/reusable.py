...
class _TagSelectorField(CustomLayout):
    def __init__(self, scrap_field_instance, _group, **kwargs):
        ...
        self.tag_selector_box = self.spawn.add_BoxLayout("vertical")
        self.tag_selector_buttons_box = self.spawn.add_BoxLayout("vertical")
        self.add_new_tag_selector_field_btn = self.spawn.add_MDIconButton("arrow-down-bold-hexagon-outline")
        self.delete_tag_selector_field_btn = self.spawn.add_MDIconButton("delete")
        self.separator_box = self.spawn.add_BoxLayout()
        self.tag_box = self.spawn.add_BoxLayout()
        self.selector_box = self.spawn.add_BoxLayout()

        # tag_box.children
        self.tag_field = self.spawn.add_MDTextField("HTML tag", "accent")
        self.parent_lbl = self.spawn.add_MDLabel("Parent")
        self.parent_chk = self.spawn.add_MDCheckbox(self.parent_lbl, group=self.parent_group)
        self.sibling_lbl = self.spawn.add_MDLabel("Children")
        self.sibling_chk = self.spawn.add_MDCheckbox(self.sibling_lbl, group=self.parent_group)
        self.find_lbl = self.spawn.add_MDLabel("Find All")
        self.find_chk = self.spawn.add_MDCheckbox(self.find_lbl)

        # selector_box.children
        self.selector_field = self.spawn.add_MDTextField("selector", "accent")
        self.cls_lbl = self.spawn.add_MDLabel("class")
        self.cls_selector_chk = self.spawn.add_MDCheckbox(self.cls_lbl, self.class_group)
        self.id_selector_lbl = self.spawn.add_MDLabel("id")
        self.id_selector_chk = self.spawn.add_MDCheckbox(self.id_selector_lbl, self.class_group)
        self.str_selector_lbl = self.spawn.add_MDLabel("string")
        self.str_selector_chk = self.spawn.add_MDCheckbox(self.str_selector_lbl, self.class_group)
    ...
    def _make(self):
        ...
        gap = self.spawn.add_gap(height_dp=None, width_dp=self.dp_single(20))

        v_separator = self.spawn.add_VSeparator()
...
class ScrapField(CustomLayout):
    def __init__(self, main_instance, getter_group, **kwargs):
        ...
        self.card = self.spawn.add_MDCard()
        self.card_inner_box = self.spawn.add_BoxLayout(orientation="vertical")
        self.card_inner_btn_box = self.spawn.add_BoxLayout()
        self.field_name = self.spawn.add_MDTextField("Custom Name", "accent")
        self.tag_selector_box_holder = self.spawn.add_BoxLayout("vertical")
        self.HSeparator = self.spawn.add_HSeparator()
        self.get_value_box = self.spawn.add_BoxLayout()

        self.mark_text_lbl = self.spawn.add_MDLabel("String")
        self.mark_text_chk = self.spawn.add_MDCheckbox(self.mark_text_lbl, self.top_group)
        self.mark_link_lbl = self.spawn.add_MDLabel("Link")
        self.mark_link_chk = self.spawn.add_MDCheckbox(self.mark_link_lbl, self.top_group)
        self.mark_email_lbl = self.spawn.add_MDLabel("Email")
        self.mark_email_chk = self.spawn.add_MDCheckbox(self.mark_email_lbl, self.top_group)
        self.delete_field_btn = self.spawn.add_MDIconButton("delete")

        self.get_lbl = self.spawn.add_MDLabel("Get")
        self.get_value_lbl = self.spawn.add_MDLabel("Value")
        self.get_value_chk = self.spawn.add_MDCheckbox(self.get_value_lbl, self.getter_group)
        self.get_href_lbl = self.spawn.add_MDLabel("href")
        self.get_href_chk = self.spawn.add_MDCheckbox(self.get_href_lbl, self.getter_group)
        self.get_str_lbl = self.spawn.add_MDLabel("text")
        self.get_str_chk = self.spawn.add_MDCheckbox(self.get_str_lbl, self.getter_group)
        self.get_title_lbl = self.spawn.add_MDLabel("title")
        self.get_title_chk = self.spawn.add_MDCheckbox(self.get_title_lbl, self.getter_group)
        self.get_src_lbl = self.spawn.add_MDLabel("src")
        self.get_src_chk = self.spawn.add_MDCheckbox(self.get_src_lbl, self.getter_group)
